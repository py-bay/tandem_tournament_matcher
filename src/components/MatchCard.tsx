...
export function MatchCard({ match, teamA, teamB }: MatchCardProps) {
    const { submitMatchResult } = useTournament();
    const [isEditing, setIsEditing] = useState(!match.isCompleted);

    // Local state for inputs before submitting
    const [board1, setBoard1] = useState<number>(match.result?.board1 ?? 0.5); // Default to draw if undefined? Or null. Let's start with null logic if possible, but number is easier.
        ...
        <div className="bg-white border border-blue-200 ring-1 ring-blue-100 rounded-lg p-4 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500 uppercase">Board 1</span>
                ...
                {/* Board 2 */}
                <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500 uppercase">Board 2</span>
                    <div className="flex gap-1">
